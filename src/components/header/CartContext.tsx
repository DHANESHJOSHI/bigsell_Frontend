...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const addToCart = (item: CartItem) => {
    setCartItems((prev) => {
      // Use productId for identification if available, otherwise fall back to id
      const identifier = item.productId || item.id;
      const existing = prev.find((i) => 
        (i.productId || i.id) === identifier && i.active === true
      );
      if (existing) {
        return prev.map((i) =>
          (i.productId || i.id) === identifier && i.active === true
  ...
  const addToWishlist = (item: CartItem) => {
    setCartItems((prev) => {
      ...
      const identifier = item.productId || item.id;
      const existing = prev.find((i) => 
        (i.productId || i.id) === identifier && i.active === false
      );
      if (existing) {
        return prev.map((i) =>
          (i.productId || i.id) === identifier && i.active === false
